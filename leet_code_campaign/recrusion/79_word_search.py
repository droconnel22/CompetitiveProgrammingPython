...
class Solution:
    def exist(self, board: List[List[str]], word: str) -> bool:
        ...
        def helper(board,word,word_index,row_index,col_index,searched):
            # We have finished the word so we are done
            if word_index == len(word):
                return True

            # Lets boundary check row
            ...
            for point in searched:
                if point[0] == row_index and point[1] == col_index:
                    return False
            
            # Check if the current cell equals our target            
            if word[word_index] != board[row_index][col_index]:
                return False
            else:
                point = [row_index,col_index]
                new_searched = searched.copy()
                new_searched.append(point)
                #print(word[word_index])
                #  can move in 4 directons, vertically or horizontally oonly
                return any([helper(board,word,word_index+1,row_index+1,col_index,new_searched), 
                helper(board,word,word_index+1,row_index-1,col_index,new_searched), 
                helper(board,word,word_index+1,row_index,col_index+1,new_searched), 
                helper(board,word,word_index+1,row_index,col_index-1,new_searched)])
           
        
        # 1. Find each cell that is the origin of the given word
        for row in range(len(board)):
            for col in range(len(board[row])):               
                if board[row][col] == word[0]:
                    searched = []
                    actual = helper(board,word,0,row,col,searched)
                    # if any return true we are done
                    if actual is True:
                        return True
        
        # if none return true it does not exist
        return False
